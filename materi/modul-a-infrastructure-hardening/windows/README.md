...
## Langkah 0 — Siapkan Domain Controller `lks.local` (GUI) — **lakukan paling dulu**
...
6. Halaman **Server Selection** > pada **Select a server from the server pool** pilih server ini (mis. `DC01`) > **Next**.
7. Halaman **Server Roles** > centang **Active Directory Domain Services** > pada popup *"Add features that are required for Active Directory Domain Services?"* klik **Add Features** > **Next**.
...
12. Halaman **DNS Options:** abaikan warning *"A delegation for this DNS server cannot be created"* > **Next**.
13. Halaman **Additional Options:** biarkan **NetBIOS domain name** default (mis. `LKS`) > **Next**.
14. Halaman **Paths:** biarkan lokasi **NTDS/SYSVOL** default > **Next** > halaman **Review Options** > **Next**.
...
> Setelah DC `lks.local` siap → lanjut ke **Modul 01–06** di bawah untuk hardening. Setiap modul punya bagian **"Panduan GUI (Langkah Klik)"** yang melanjutkan dari titik ini.
